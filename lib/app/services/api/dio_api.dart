...
class TrkoRepository{
  ...
  Future<List<Project>> clientProject({String token, String clientId})async{

    print("We got token for clients project: $token and $clientId");

    Map<String, String> headerDetails = {"Authorization" : "Bearer $token"};

    String url = baseUrl + "/project/client/" + clientId;
    ...
    try{
      ...
      for(Map project in response.data["data"]){
      ...
      debugPrint("onCall Clients List Error: $e");
  ...
  Future projectPayment({String token, String projectId, String clientId})async{

    print("We got token for clients updates: $token and clientId: $clientId and projectId $projectId");

    Map<String, String> headerDetails = {"Authorization" : "Bearer $token"};

    String url = baseUrl + "/payment/project/" + projectId + "/client/" + clientId;
    ...
    try{
      ...
      print("got payments $response");

      for(Map payment in response.data["data"]){
        print("$payment");
        paymentList.add(Payment.fromMap(payment));
      }
      ...
      print("onCall Clients payment List Error: ${e.response}");

      if(e.response == null){
        print("onCall payments List DioErrorType.Default: ${e.response}");
        paymentList.add(Payment(message: "Oops! check your internet connection"));
        return paymentList;
      }
      else{
        print("onCall payments List DioErrorType.RESPONSE: ${e.response.data}");
        paymentList.add(Payment(statusCode: e.response.data["status"], message: e.response.data["message"]));
        return paymentList;
      }
  ...
  Future addPayment({Map<String, dynamic> paymentData, String token})async{

    print("We got token for clients updates: $token and paymentData: $paymentData");
    ...
    try{
      ...
      response = await client.post(url, data: paymentData, options: Options(headers: headerDetails));

      print("added $response");

      return CallResponse(statusCode: response.data["status"], message: "Payment has been successfuly added");

    }
    catch (e){

      print("onCall Clients add payment List Error: ${e.response}");
  ...
  Future editPayment({List<Map> paymentData, String token, String paymentId})async{

    print("We got token for clients updates: $token and paymentData: $paymentData");

    Map<String, String> headerDetails = {"Authorization" : "Bearer $token"};

    String url = baseUrl + "/payment/" + paymentId.toString();

    try{

      response = await client.put(url, data: paymentData, options: Options(headers: headerDetails));

      print("edited $response");

      return CallResponse(statusCode: response.data["status"], message: "Payment has been successfuly modified");

    }
    catch (e){

      print("onCall Clients edit payment Error: ${e.response}");
  ...
  Future<CallResponse> deletePayment({String paymentId, String token})async{

    print("We got token for clients updates: $token and paymentId: $paymentId");
